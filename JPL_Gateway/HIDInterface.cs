...
namespace HIDInterface
{
    internal class HIDDevice
    {
        ...
        public struct interfaceDetails
        {
            ...
            public string InterfaceClassGuid;
            public UInt16 Usage;
        ...
        public static Dictionary<string, interfaceDetails> dictionary = new Dictionary<string, interfaceDetails>();
        ...
        public static interfaceDetails[] getConnectedDevices()
        {
            ...
            while (b)
            {
                ...
                //Console.WriteLine("DEBUG :: {0}", devicePath);
                if (devicePath.Contains("vid_2ea1") || devicePath.Contains("vid_0d8c")
                    || devicePath.Contains("vid_0b0e") || devicePath.Contains("vid_3015")
                    || devicePath.Contains("vid_05a6") || devicePath.Contains("vid_33cb"))
                {
                    // Console.WriteLine("DEBUG :: {0}", devicePath);
                    // Console.WriteLine("DEBUG :: {0}", devIface.InterfaceClassGuid.ToString());
                    ...
                    productInfo.InterfaceClassGuid = devIface.InterfaceClassGuid.ToString();
                    productInfo.Usage = capabilities.Usage;
                    productInfo.UsagePage = capabilities.UsagePage;

                    if (productInfo.VID == 0x2EA1 || productInfo.VID == 0x0B0E || productInfo.VID == 0x3015
                        || productInfo.VID == 0x05A6 || productInfo.VID == 0x33CB)
                    {
                        if (productInfo.UsagePage != 0x0B)
                        {
                            if (productInfo.PID != 0x0422 && productInfo.PID != 0x0423 && productInfo.PID != 0x0425 && productInfo.PID != 0x0A01)
                                continue;

                            if (productInfo.UsagePage != 0x0C)
                                continue;
                        }
                    }
                    if (!dictionary.ContainsKey(productName))
                    {
                        //Console.WriteLine("{0}", manfString);
                        //Console.WriteLine("Name={0}", productName);
                        dictionary.Add(productName, productInfo);
        ...
        public HIDDevice(string devicePath, bool useAsyncReads)
        {
            ...
            Console.WriteLine("deviceConnected {0}", deviceConnected);
            if (!deviceConnected)
            {
        ...
        private void initDevice(string devicePath, bool useAsyncReads)
        {
            ...
            // JONATHAN FS_read = new FileStream(handle_read, FileAccess.ReadWrite, capabilities.OutputReportByteLength, false);
        ...
        public void write(byte[] data)
        {
            if (data.Length > capabilities.InputReportByteLength)
                throw new Exception("Output report must not exceed " + (capabilities.InputReportByteLength - 1).ToString() + " bytes");

            //uint numBytesWritten = 0;
            byte[] packet = new byte[capabilities.InputReportByteLength];
        ...
        private void GetInputReportData(IAsyncResult ar)
        {
            ...
            //triggers the event to be heard by the calling class
        }
